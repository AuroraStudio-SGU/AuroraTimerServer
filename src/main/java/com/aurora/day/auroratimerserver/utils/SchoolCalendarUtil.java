...
public class SchoolCalendarUtil {
    ...
    public static TermTime getTermTime(){
        ...
        String calePage = CalePageRequest.execute().body();
    ...
    private static TermTime toTerm(List<Object> titles){
        ...
        for (Object title : titles) {
            ...
            while (matcher.find()) {
                ...
                Date start = strToDate(matcher.group(3),type,false);
                Date end = strToDate(matcher.group(4),type,true);
    ...
    private static Date strToDate(String uncommonDateStr,String type,boolean isEndDate){
        int year = DateUtil.year(new Date());
        if(type.equals("寒假") && isEndDate) year++;
